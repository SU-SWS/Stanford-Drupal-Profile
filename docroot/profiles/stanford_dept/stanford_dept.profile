...
function stanford_dept_sites_tasks() {
  /**
   * General stuff.
   */
  /**
  ...
  if($auth_method == 'simplesamlphp') {
    ...
    if(stanford_dept_sites_hosted()) {
      module_enable(array('stanford_afs_quota'));
      stanford_dept_sites_add_webauth_user(
 ...
 */
function stanford_dept_sites_hosted() {
 ...
 *
 */
function stanford_dept_sites_add_webauth_user($sunet, $name = '', $email = '') {
